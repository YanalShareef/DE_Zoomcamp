...
def main(args):
    ...
    csv_name = "output.parqet"

    # download the csv file 
    os.system(f"wget {file_url} -O {csv_name}")

    # Read Parquet file into Pandas DataFrame
    trips = pq.read_table(args.fileurl)
    data = trips.to_pandas()

    # Create a PostgreSQL engine using the provided arguments
    engine_url = f"postgresql://{args.user}:{args.password}@{args.host}:{args.port}/{args.databasename}"
    ...
    dfs[0].head(0).to_sql(name=args.tablename, con=engine, if_exists="replace")
